...
class PoolManagerTest extends TestCase
{
    ...
    public function testCreatePoolWithCapacity(): void
    {
        ...
        // Assert
        $this->assertTrue(
            $this->poolManager->exists($this->owner, 'default'),
    ...
    public function testRemoveSpecificPoolType(): void
    {
        ...
        $this->assertTrue(
            $this->poolManager->exists($this->owner, 'readonly'),
        ...
        $this->eventDispatcher
            ...
            ->willReturnCallback(function ($event) {
                ...
                if ($callCount === 1) {
                    ...
                    $this->assertSame(10, $event->capacity);
                    $this->assertSame(0.0, $event->timeout);
                }
                return $event;
            });
    ...
    public function testAcquireDispatchesResourceBusyEventOnTimeout(): void
    {
        ...
        \Swoole\Coroutine\run(function () {
            ...
            $container = new \Switon\Testing\Container();
    ...
    public function testReleaseDispatchesReturnFailedEventWhenPoolNotFound(): void
    {
        ...
        $this->eventDispatcher
            ...
            ->with($this->callback(function ($event) {
                ...
                $this->assertSame('default', $event->type);
                $this->assertSame('Pool not found when returning resource', $event->message);
                $this->assertSame($this->owner, $event->owner);
                return true;
            }));
    ...
    public function testReleaseDispatchesReturnFailedEventWhenPoolAlreadyFull(): void
    {
        ...
        $this->eventDispatcher
            ...
            ->with($this->callback(function ($event) {
                ...
                $this->assertSame('default', $event->type);
                $this->assertSame('Pool is full when returning resource', $event->message);
                $this->assertSame($this->owner, $event->owner);
                return true;
            }));
    ...
    public function testAddAutoCreatedPoolUsesRequestedSizeAsCapacityBoundary(): void
    {
        $this->poolManager->add($this->owner, new TestConnection('only'), 1, 'ephemeral');

        $this->assertSame(1, $this->poolManager->size($this->owner, 'ephemeral'));
        $this->expectException(FullException::class);
        $this->poolManager->add($this->owner, new TestConnection('overflow'), 1, 'ephemeral');
    }
}
